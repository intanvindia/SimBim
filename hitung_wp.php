...
// Tentukan link dashboard berdasarkan level user
$dashboard_link = 'dashboard.php'; // Default untuk orang tua
...
if ($id_asesmen <= 0) die("ID Asesmen tidak valid.");
...
if ($res_mhs->num_rows == 0) {
    $stmt_mhs->close();
    die("<div style='padding:20px; color:red;'>Data riwayat asesmen tidak ditemukan.</div>");
}
...
// =========================================================
// SIMPAN HASIL REKOMENDASI UTAMA KE TABEL LOG
// =========================================================
if (!empty($ranking_aktif)) {
    // Ambil ID kelas dengan ranking teratas
    $top_class_id = array_key_first($ranking_aktif);

    // Simpan atau perbarui log hasil
    $stmt_log = $conn->prepare("INSERT INTO rekomendasi_hasil (id_asesmen, id_kelas) VALUES (?, ?) ON DUPLICATE KEY UPDATE id_kelas = ?");
    $stmt_log->bind_param("iii", $id_asesmen, $top_class_id, $top_class_id);
    $stmt_log->execute();
    $stmt_log->close();
}
...
while($kr = $res_kriteria->fetch_assoc()) {
    ...
    // Ambil rata-rata skor kuesioner dari Step 1
...
$page_title = "Hasil Analisis Rekomendasi - SIMBIM";
include 'includes/header.php';
?>

<div class="container">
    <div class="py-4">
        <h2 class="fw-bold text-dark">🎯 Hasil Analisis Rekomendasi Kelas Bimbel</h2>
        <p class="text-muted">Rekomendasi dihitung menggunakan kecocokan multi-kriteria berbasis metode ilmiah Weighted Product.</p>
    </div>
    ...
    <div class="alert alert-info border-0 shadow-sm p-4">
        ...
        📅 <strong>Tanggal Pelaksanaan Tes:</strong> <?= date('d F Y', strtotime($info_anak['tgl_asesmen'])); ?>
    </div>
    ...
    <div class="pb-card p-4 mb-5 border-0 shadow-sm rounded-4 text-center">
        ...
        <p class="text-muted small mt-3 italic">Grafik ini menunjukkan sebaran kecenderungan minat dan bakat anak berdasarkan jawaban kuesioner.</p>
        ...
        <div class="mt-4 p-3 bg-light rounded-3 border text-start mx-auto" style="max-width: 600px;">
            ...
            <p class="mb-0">Berdasarkan hasil asesmen, Ananda <strong><?= htmlspecialchars($info_anak['nama_anak']); ?></strong> memiliki kecenderungan potensi paling menonjol pada bidang <strong><?= $highest_name; ?></strong> dengan skor rata-rata <strong><?= $highest_val; ?></strong> dari skala 5. Hal ini menunjukkan minat yang kuat yang dapat dioptimalkan melalui pemilihan kelas yang tepat.</p>
        </div>
    </div>

    <h3 class="h5 fw-bold mt-4 mb-3">1. Pilihan Program Kelas Utama (Siap Daftar)</h3>
        ...
        <tbody>
            ...
            foreach($ranking_aktif as $ra) { 
                ...
                <tr class="<?= $is_top ? 'table-success fw-bold' : ''; ?>">
                    ...
                    <td><?= $ra['nama']; ?> <?= ($no == 2) ? "🌟 (Paling Direkomendasikan)" : ""; ?></td>
    ...
    <div class="card bg-warning bg-opacity-10 border-warning border-opacity-25 mt-5 p-4 rounded-4 shadow-sm">
        <h3 class="h5 fw-bold text-warning-emphasis">💡 Potensi Kelas Ekstensi (Coming Soon)</h3>
        <p class="small text-muted mb-3">Berdasarkan kecenderungan minat yang diisi, berikut adalah urutan kecocokan Ananda untuk program yang akan segera diluncurkan:</p>
                ...
                <div class="col">
                    <div class="card border-0 shadow-sm h-100">
                        <div class="card-body py-2">
                            <span class="fw-bold text-primary">#<?= $no_soon++; ?></span> <?= $rc['nama']; ?>
                        </div>
                    </div>
                </div>
            <?php } ?>
...
<script>
    // Gunakan window.myRadarChart agar pasti bisa diakses dari fungsi handleCetak
    ...
    window.onload = function() {
        ...
        data: {
            ...
            datasets: [{
                label: 'Skor Potensi Ananda (Skala 1-5)',
        ...
        options: {
            ...
            animation: false, // Mematikan animasi agar konversi ke Base64 (PDF) stabil
            layout: {
    ...
    function handleCetak() {
        ...
        if (window.myRadarChart) {
            try {
                // Ambil gambar chart dalam format Base64
                ...
                if (imageBase64 === "data:," || !imageBase64) {
                    alert("Grafik belum siap. Harap tunggu hingga grafik muncul sempurna.");
                    return;
                }

                // Isi nilai ke input hidden dan kirim form
                ...
                alert("Gagal memproses gambar grafik.");
            }
        } else {
